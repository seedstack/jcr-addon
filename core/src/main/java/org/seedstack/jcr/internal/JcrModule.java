...
class JcrModule extends AbstractModule {
    ...
    protected void configure() {
        ...
        bindInterceptor(Matchers.any(), getMethodMatcher(), new JcrSessionInterceptor(sessionLink, configuration));
        configuration.getRepositories()
                .forEach((key, value) -> bindConfiguration(key,
                        value,
    ...
    private void bindConfiguration(String name, RepositoryConfig configuration, boolean isDefault) {
        ...
        sessionLink.registerProvider(new JcrSessionProvider(name, configuration, this.factoryInstances));
    ...
    private static Matcher<Method> getMethodMatcher() {
